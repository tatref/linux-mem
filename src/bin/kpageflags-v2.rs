...
async fn async_client(remote: SocketAddr) {
    ...
    let mut mouse_world: Vec2 = Vec2::ZERO;
    ...
    'mainloop: loop {
                ...
                Message::FirstUpdate(message) => {
                    ...
                    for (start, end, _) in &update.as_ref().unwrap().memory_segments {
                        for pfn in start.0..end.0 {
                            ...
                            .unwrap();
                            let (x, y) = fast_hilbert::h2xy::<u64>(index.into(), order.unwrap());
            ...
            && mouse_world.y < img.as_ref().unwrap().height() as f32
        {
            ...
            if let Some(pfn) = pfn {
                ...
                for (pfn_start, pfn_end, segment_flags) in &update.as_ref().unwrap().memory_segments
                {
...
fn main() {
    ...
    let port: u16 = args.get(1).unwrap_or(&"10000").parse().unwrap();
    ...
    let mut segments = get_memory_zones_flags(&iomem, &mut kpageflags);
    let mut all_processes: Vec<ProcessInfo> = get_all_processes_info();
    ...
    let (tx, rx): (SyncSender<UpdateMessage>, Receiver<UpdateMessage>) = mpsc::sync_channel(1);
    ...
    let _worker_thread = thread::spawn(move || {
        ...
        loop {
            ...
            let memory_segments = get_memory_zones_flags(&iomem, &mut kpageflags);
            let processes_info = get_all_processes_info();
            ...
            tx.send(update_message).expect("Can't send update_message");
    ...
    loop {
        ...
        if let (Ok(data), true) = (rx.try_recv(), autorefresh) {
            ...
            (segments, all_processes) = (data.memory_segments, data.processes_info);
